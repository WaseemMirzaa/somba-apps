...
export default function SellerPromotionCreatePage() {
  ...
  return (
    <div className="space-y-6">
      <PageHeader title={t("promotionCreate")} subtitle={fr ? "Sélectionnez produits, réduction, planning" : "Select products, set discount, schedule"} backHref="/seller/promotions" />
      <DetailSection title={fr ? "Détails campagne" : "Campaign Details"}>
        <div className="grid gap-4 sm:grid-cols-2">
          <input className="rounded-lg border border-sky-200 px-4 py-2 text-sm" placeholder={fr ? "Nom de campagne" : "Campaign Name"} value={form.name} onChange={(e) => setForm({ ...form, name: e.target.value })} />
        ...
        <button
          onClick={() => {
            if (!form.name.trim()) { toast(fr ? "Entrez un nom de campagne" : "Enter a campaign name", "error"); return; }
