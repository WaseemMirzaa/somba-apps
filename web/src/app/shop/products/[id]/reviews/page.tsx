...
export default function ProductReviewsPage() {
  ...
  if (!product) return <div className="text-center text-slate-500">Product not found</div>;

  return (
    <div className="mx-auto max-w-3xl space-y-8">
      <PageHeader
        title={`Reviews — ${product.name}`}
        subtitle={`${product.rating} ★ · ${product.reviews.toLocaleString()} reviews`}
        backHref={`/shop/products/${id}`}
        actions={<Button size="sm" onClick={() => setShowForm(true)}>Write Review</Button>}
      />
      ...
      {showForm && (
        <div className="card-premium space-y-4 p-6">
          ...
          <textarea className="input-premium w-full px-4 py-3 text-sm" rows={4} placeholder="Share your experience..." value={reviewText} onChange={(e) => setReviewText(e.target.value)} />
          <div className="rounded-xl border-2 border-dashed border-[var(--border)] p-8 text-center text-sm text-slate-500">
            Upload photos (mock) — drag & drop or click
          </div>
          <Button
            onClick={() => {
              if (!rating || !reviewText.trim()) { toast("Please add a rating and review", "error"); return; }
              setExtraReviews((r) => [...r, { author: "You", rating, text: reviewText, date: "Just now" }]);
              setShowForm(false);
              setRating(0);
              setReviewText("");
