...
class BatchOverviewScreen extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      appBar: backAppBar(context, tr(context, 'Batch BAT-204')),
      body: ListView(padding: const EdgeInsets.fromLTRB(16, 8, 16, 24), children: [
        Container(
          padding: const EdgeInsets.all(18),
          decoration: BoxDecoration(gradient: AppColors.brandGradient, borderRadius: BorderRadius.circular(22)),
          child: Row(children: [
            _hstat('${stops.length}', tr(context, 'Stops')),
            _hdiv(),
            _hstat('12.4', 'km'),
            _hdiv(),
            _hstat('$items', tr(context, 'Items')),
          ]),
        ),
        const SizedBox(height: 16),
        Text(tr(context, 'Stops in order'), style: const TextStyle(fontWeight: FontWeight.w800, fontSize: 15)),
        ...
        ...List.generate(stops.length, (i) {
          ...
          return Padding(padding: const EdgeInsets.only(bottom: 12), child: SurfaceCard(padding: const EdgeInsets.all(14), child: Row(children: [
            ...
            Expanded(child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
              ...
              Text('${tr(context, t.typeLabel)} · ${t.distanceKm} km', style: const TextStyle(color: AppColors.muted, fontSize: 12.5)),
            ])),
            Pill('${t.items} ${tr(context, t.items > 1 ? 'items' : 'item')}', color: AppColors.primary.withValues(alpha: 0.12), textColor: AppColors.primary, fontSize: 10.5),
          ])));
        }),
        const SizedBox(height: 4),
        PrimaryButton(tr(context, 'Start batch'),
            icon: Icons.play_arrow_rounded,
            onPressed: () {
              ScaffoldMessenger.of(context).showSnackBar(SnackBar(content: Text(tr(context, 'Batch BAT-204 started'))));
...
class ZoneScreen extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    final zones = [
      ('Gombe', tr(context, 'Very high demand'), 0.95, AppColors.danger),
      ('Limete', tr(context, 'High demand'), 0.7, AppColors.accent),
      ('Ngaliema', tr(context, 'Medium demand'), 0.45, AppColors.info),
      ('Kalamu', tr(context, 'Low demand'), 0.2, AppColors.primary),
    ];
    return Scaffold(
      appBar: backAppBar(context, tr(context, 'Zones & demand')),
      body: ListView(padding: const EdgeInsets.fromLTRB(16, 8, 16, 24), children: [
        SurfaceCard(child: Row(children: [
          ...
          const SizedBox(width: 12),
          Expanded(child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
            Text(tr(context, 'Current zone · Gombe'), style: const TextStyle(fontWeight: FontWeight.w800, fontSize: 14.5)),
            Text(tr(context, 'You are online here'), style: const TextStyle(color: AppColors.muted, fontSize: 12.5)),
          ])),
          Pill(tr(context, 'Online'), color: AppColors.primary.withValues(alpha: 0.14), textColor: AppColors.primary),
        ])),
        const SizedBox(height: 16),
        Text(tr(context, 'Live demand'), style: const TextStyle(fontWeight: FontWeight.w800, fontSize: 15)),
        const SizedBox(height: 10),
        ...
        const SizedBox(height: 4),
        PrimaryButton(tr(context, 'Move to high-demand zone'),
            icon: Icons.trending_up_rounded,
            onPressed: () => ScaffoldMessenger.of(context).showSnackBar(SnackBar(content: Text(tr(context, 'Rerouting you toward Gombe…'))))),
      ]),
    );
  }
}
